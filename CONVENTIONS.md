...
| Test-only modules in the production tree | `*.test-fixture.ts`, and **never imported from production** — the suffix is the boundary. Two sweeps in `file-saver.test.ts` hold it jointly: the invariant-17 `fs` sweep skips any filename containing `.test-fixture.`, and *nothing in production imports a test-only module* is what makes the rule enforced rather than habitual. Remove either and the other reports an empty offender list, which reads exactly like compliance. Both key on one shared predicate (`TEST_ONLY`); the residual it does not cover is stated at `file-saver.test.ts::moduleSpecifiers` | `curl-output.test-fixture.ts`, `cpu-time.test-fixture.ts` |
