...
- **Branch protection / required checks:** work happens on feature branches off `main`; PRs go to `main`, which **is** protected. Verified against the API on 2026-09-01 rather than assumed:
  - **0 approving reviews required**, and `require_code_owner_reviews` is **off**. Re-read from the API on 2026-09-07 (`required_approving_review_count: 0`, `require_code_owner_reviews: false`).
  ...
  - **`required_status_checks` is `null`** — CodeQL, `Analyze (javascript-typescript)` and CodeRabbit run on PRs but **none of them gates a merge**. Re-read from the API on 2026-09-07.

  > **This section has now been wrong in both directions, and the second error is the one worth keeping in mind.** It first stated that no branch protection was configured — written from assumption, asserting a *weaker* posture than the repository had. The correction then overshot and claimed **1 approving review required** with `require_code_owner_reviews` on, which asserted a *stronger* posture than the repository has: the API returns 0 required approvals, no code-owner requirement, and `required_status_checks: null`. Corrected against the API on 2026-09-07.
  >
  > **The second error is the more dangerous shape.** An under-claim gets challenged the first time someone relies on it; an over-claim is believed, and the "Accepted merge gate" below rested on it — concluding that GitHub enforced a separate human approver when nothing does. **Re-read this from the API, not from this file, before relying on any of it.**
- **Accepted merge gate:** **nothing but the operator's own authorisation gates a merge to `main`.** GitHub requires 0 approving reviews and no status check is required, so the intended posture — bot review (Surface 3) plus explicit human authorisation — is enforced **by discipline, not by the platform**. Signed commits and the push restriction are the only mechanical controls. If a PR carries an unresolved Surface 3 finding on a high-surface trigger from §7, there is no gate that will stop it; that is the moment the discipline is the whole control.
