 ...
 */
const LINUX_BLOCKED_DIRS = Object.freeze([
    "/etc",
    "/sys",
    "/proc",
    "/dev",
 ...
 */
const MACOS_BLOCKED_DIRS = Object.freeze([
    "/System",
    "/Library",
    "/private/etc",
    "/private/var",
 ...
 */
const MACOS_ROOT_ONLY_BLOCKED = Object.freeze([
    "/Volumes",
]);
/**
 ...
 */
const WINDOWS_BLOCKED_PATTERNS = Object.freeze([
    /^[a-z]:\\windows(\\|$)/i,
    /^[a-z]:\\program files(\\|$)/i,
 ...
 */
export function isBlockedSystemDirectory(resolvedPath) {
 ...
 */
export function createBlockedDirectoryError(originalPath, resolvedPath) {
    const pathInfo = originalPath === resolvedPath
        ? `"${originalPath}"`
        : `"${originalPath}" (resolves to "${resolvedPath}")`;
    return new Error(`Invalid output_dir ${pathInfo}: writing to system directories is not allowed. ` +
