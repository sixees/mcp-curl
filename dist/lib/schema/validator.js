 ...
 */
export function validateApiSchema(data) {
    ...
    if (!result.success) {
        ...
        throw new ApiSchemaValidationError(`API schema validation failed:\n${messages.join("\n")}`, result.error.issues);
    }
    // Additional validation: check for duplicate endpoint IDs
    const endpointIds = new Set();
    for (const endpoint of result.data.endpoints) {
        if (endpointIds.has(endpoint.id)) {
            throw new ApiSchemaValidationError(`Duplicate endpoint ID: ${endpoint.id}`, []);
        }
    ...
    for (const endpoint of result.data.endpoints) {
        ...
        const definedPathParams = new Set((endpoint.parameters || [])
            .filter((p) => p.in === "path")
            .map((p) => p.name));
        for (const pathParam of pathParams) {
            const paramName = pathParam.slice(1, -1); // Remove { and }
            if (!definedPathParams.has(paramName)) {
                throw new ApiSchemaValidationError(`Path parameter {${paramName}} in endpoint "${endpoint.id}" is not defined in parameters`, []);
            }
        }
    }
    return result.data;
