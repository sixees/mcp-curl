...
describe("validateApiSchema", () => {
    ...
    it("rejects invalid apiVersion", () => {
        expect(() => validateApiSchema({ ...validSchema, apiVersion: "2.0" })).toThrow(ApiSchemaValidationError);
    });
    it("rejects invalid baseUrl", () => {
        expect(() => validateApiSchema({
            ...validSchema,
            api: { ...validSchema.api, baseUrl: "not-a-url" },
        })).toThrow(ApiSchemaValidationError);
    });
    it("rejects empty endpoints array", () => {
        expect(() => validateApiSchema({ ...validSchema, endpoints: [] })).toThrow(ApiSchemaValidationError);
    });
...
`;
    ...
    it("throws on invalid YAML syntax", () => {
        expect(() => loadApiSchemaFromString("invalid: yaml: content:")).toThrow(ApiSchemaLoadError);
    });
...
describe("buildUrl", () => {
    ...
    it("substitutes path parameters", () => {
        const url = buildUrl("https://api.example.com", "/items/{id}", { id: "123" }, {});
    ...
    it("appends query parameters", () => {
        const url = buildUrl("https://api.example.com", "/items", {}, { page: "1", limit: "10" });
    ...
    it("handles multiple path parameters", () => {
        const url = buildUrl("https://api.example.com", "/users/{userId}/posts/{postId}", { userId: "42", postId: "99" }, {});
