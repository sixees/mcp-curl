 ...
 */
export type BeforeRequestResult<T> = void | {
    params?: Partial<T>;
} | {
    shortCircuit: true;
    response: string;
 ...
 */
export type BeforeRequestHook<T = CurlExecuteInput | JqQueryInput> = (ctx: HookContext<T>) => BeforeRequestResult<T> | Promise<BeforeRequestResult<T>>;
/**
 * Hook called after successful tool execution.
 * Receives the response for logging, metrics, caching, etc.
 */
export type AfterResponseHook<T = CurlExecuteInput | JqQueryInput> = (ctx: HookContext<T> & {
    response: string;
    isError: boolean;
}) => void | Promise<void>;
/**
 ...
 */
export type OnErrorHook<T = CurlExecuteInput | JqQueryInput> = (ctx: HookContext<T> & {
    error: Error;
}) => void | Promise<void>;
/**
