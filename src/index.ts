...
async function validateOutputDir(dir: string): Promise<void> {
    // Check for .. components in the original input (even if resolved path is valid)
    if (dir.includes("..")) {
        throw new Error(
            `Invalid output_dir: path contains ".." which is not allowed for security. ` +
...
async function validateFilePath(filepath: string): Promise<void> {
    ...
    if (filepath.includes("..")) {
        throw new Error(
            `Invalid filepath: path contains ".." which is not allowed for security. ` +
            `Please provide an absolute path without relative components.`
        );
    }
